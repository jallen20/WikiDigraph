...
public class DirectedGraph<T extends Comparable<T>> extends Graph<T> {
	...
	public Map<T, T> dijkstra(T start) {
		...
		var dist = new TreeMap<T, Double>();
		var prev = new TreeMap<T, T>();
		var unchecked = new TreeSet<T>(super.nodeSet());

		dist.put(start, 0.0);
		for (var current : this.nodeSet()) {
		...
		while (!unchecked.isEmpty()) {
			var u = getShortestDistance(unchecked, dist);
			unchecked.remove(u);
			for (var v : getNeighbors(u)) {
				if (unchecked.contains(v)) {
					var distance = dist.get(u) + getWeight(u, v);
